...
include("fixtures/lifecycle_stencil_support.jl")

@testset "seed-stencil lifecycle placement uses Cartesian domain" begin
    test_seed_stencil_lifecycle(CorePotts.SequentialProgramEngine())
    test_seed_stencil_lifecycle(CorePotts.CheckerboardProgramEngine())
end
...
@testset "remove-cell settlement preserves the selected medium owner" begin
    ...
    lifecycle_mode = CorePotts.HostLifecycleExecution()
    @test !CorePotts._stage_owner_change!(
        lifecycle_mode,
        CorePotts._ownership_transfer_recipe(restored, plan),
        CorePotts._lifecycle_owner_change_state(
            lifecycle_mode, restored, restored.lifecycle_workspace,
        ),
        nothing, 36, Int32(0),
    )
