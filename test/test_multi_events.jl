...
@testset "AbstractMultiEvent Routing" begin
    ...
    CorePotts.get_event_args(::TestSubEventA, mask, u, p, cache, t) = begin
        global test_execution_counts[:A] += 1;
        (mask,)
    end
    CorePotts.get_event_args(::TestSubEventB, mask, u, p, cache, t) = begin
        global test_execution_counts[:B] += 1;
        (mask,)
    end
    CorePotts.get_event_args(::TestSubEventC, mask, u, p, cache, t) = begin
        global test_execution_counts[:C] += 1;
        (mask,)
    end
    ...
    CorePotts.get_sub_events(::TestMultiEvent) = (
        TestSubEventA(), TestSubEventB(), TestSubEventC())
