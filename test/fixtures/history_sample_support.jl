...
function _initial_history_program(engine; backend = CorePotts.CPUProgramBackend())
    C = CorePotts
    ...
    program = capability_test_program(program; backend)
...
function _history_feedback_program(engine; backend = CorePotts.CPUProgramBackend(), extra_write = false, projected_target = false, ownership_write = false, constraint_read = nothing, lifecycle_write = false)
    C = CorePotts
...
function test_initial_history_capture(engines; backend = CorePotts.CPUProgramBackend(), adapt_runtime = identity)
    return @testset "initial history capture preserves prehistory and does not execute a boundary" begin
        C = CorePotts
        for engine in engines
            (; program, initial, source, initial_history, periodic_history) = _initial_history_program(engine; backend)
            runtime = adapt_runtime(C.initialize_program(program, initial, Float32[], UInt64(17), UInt32(1)))
            ...
            deferred = adapt_runtime(C.initialize_program(program, initial, Float32[], UInt64(17), UInt32(1); capture_initial_history = false))
            @test vec(C.state_block(C.program_snapshot(deferred).descriptor_state, initial_history).values) == Float32[1, 2, 3]
            ...
            nonzero = adapt_runtime(C.initialize_program(program, initial, Float32[], UInt64(17), UInt32(1); initial_mcs = 3))
            @test vec(C.state_block(C.program_snapshot(nonzero).descriptor_state, initial_history).values) == Float32[1, 2, 3]
        end
    end
end

function test_history_initialization_failure(; backend = CorePotts.CPUProgramBackend(), adapt_runtime = identity)
    return @testset "failed initialization abandons a sampled candidate without publishing MCS zero" begin
        C = CorePotts
        (; program, initial, initial_history) = _initial_history_program(C.CheckerboardProgramEngine(); backend)
        runtime = adapt_runtime(C.initialize_program(program, initial, Float32[], UInt64(17), UInt32(1); capture_initial_history = false))
        ...
        fill!(
            workspace.state.program_status, C.ProgramStatus(
...
function test_history_feedback(engines; backend = CorePotts.CPUProgramBackend(), adapt_runtime = identity)
    return @testset "two retained model samples drive the same ordinary boundary evaluator" begin
        C = CorePotts
        for engine in engines
            (; program, fixture) = _history_feedback_program(engine; backend)
