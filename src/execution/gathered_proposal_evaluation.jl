...
@inline _execute_proposal_scalar(
    ::_ExecutableParameter{Index}, context
) where {Index} =
    getfield(_proposal_parameters(context), Index)
@inline _execute_proposal_scalar(
    value::_ExecutableStateReference, context
) = value
@inline _execute_proposal_scalar(
    value::_ExecutableTrackerKey, context
) = value
...
@inline _execute_proposal_scalar(
    ...
    _gathered_proposal(context).new_owner <= 0
@inline _execute_proposal_scalar(
    ::_ExecutableProposalContext{:energy_anchor_site}, context
) = context.anchor
@inline _execute_proposal_scalar(
    ::_ExecutableProposalContext{:energy_anchor_cell}, context
) = context.anchor
@inline _execute_proposal_scalar(
    ::_ExecutableProposalContext{:energy_anchor_contact}, context
) = context.anchor
@inline _execute_proposal_scalar(
...
@inline function _execute_proposal_ordered(
        operation, arguments::Tuple{A}, context
    ) where {A}
    return operation(_execute_proposal_scalar(first(arguments), context))
end
@inline function _execute_proposal_ordered(
        operation, arguments::Tuple{A, B}, context
    ) where {A, B}
...
@inline function _execute_proposal_ordered(
        operation, arguments::Tuple{A, B, C, Vararg}, context
    ) where {A, B, C}
...
@inline function _execute_proposal_scalar(
        call::_ExecutableIntegerPower{N}, context
    ) where {N}
...
@inline function _execute_proposal_scalar(
        call::_ExecutableContextualCall{
            ResourceOperation{:bounded_fold}, Tuple{A, B, C, D},
        }, context,
    ) where {A, B, C, D}
...
@inline function _execute_proposal_scalar(
        call::_GatheredQualifiedTrackerCall{Quantity}, context
    ) where {Quantity}
    arguments = _execute_proposal_arguments(call.arguments, context)
...
@inline apply_resource_operation(
    ...
    context::_GatheredProposalContext
) = _gathered_bounded_fold(arguments, context)
@inline apply_resource_operation(
    ::ResourceOperation{:bounded_fold}, arguments,
    context::_GatheredAnchorEnergyContext
) = _gathered_bounded_fold(arguments, context)
...
@inline function _gathered_relationship_endpoint(
        context::_GatheredAnchorEnergyContext, ::Val{Endpoint}
    ) where {Endpoint}
...
@inline function apply_resource_operation(
    ...
    1 <= payload_slot <= length(resource.payload) || throw(
        ArgumentError(
...
@inline function _gathered_owner_volume(
        proposal::_GatheredProposalContext,
        owner::Int32
    )
    owner <= 0 && return Int32(0)
...
@inline function _proposal_term_evaluation(
        term::_ExecutableProposalTerm{
            E, <:HamiltonianRole{
                <:SiteEnergyDomainPlan, <:TargetSiteAffectedPlan,
            },
        }, context,
        ::Type{T}
    ) where {E, T <: AbstractFloat}
...
@inline function _proposal_term_evaluation(
        term::_ExecutableProposalTerm{
            E, <:HamiltonianRole{
                <:SiteEnergyDomainPlan, <:NeighborhoodSitesAffectedPlan,
            },
        }, context,
        ::Type{T}
    ) where {E, T <: AbstractFloat}
    ...
    while lane < count
        ...
        if center > 0 && center != target
            ...
            while prior < lane
                duplicate |= @inbounds(
                    context.reverse_contact_sites[Int(start + prior)]
                ) == center
                prior += Int32(1)
            end
            if !duplicate
...
@inline function _proposal_term_evaluation(
        term::_ExecutableProposalTerm{
            E, <:HamiltonianRole{
                <:CellEnergyDomainPlan, <:SourceTargetCellsAffectedPlan,
            },
        }, context,
        ::Type{T}
    ) where {E, T <: AbstractFloat}
...
@inline function _proposal_term_evaluation(
        term::_ExecutableProposalTerm{
            E, <:HamiltonianRole{
                <:ContactEnergyDomainPlan, <:IncidentContactsAffectedPlan,
            },
        }, context,
        ::Type{T}
    ) where {E, T <: AbstractFloat}
    ...
    while lane <= length(resource.active)
        ...
        if active_sample.present && something(active_sample.value)
            ...
            if !duplicate
                ...
                before = before_present ? T(
                        _execute_proposal_scalar(
                            term.evaluator,
                            _GatheredAnchorEnergyContext(
                                context, false, edge, handle
                            )
                        )
                    ) : zero(T)
                after = after_present ? T(
                        _execute_proposal_scalar(
                            term.evaluator,
                            _GatheredAnchorEnergyContext(
                                context, true, edge, handle
                            )
                        )
                    ) : zero(T)
                delta += after - before
...
@inline function _fold_executable_proposal_numeric_terms(
    ...
    contribution = _proposal_numeric_evaluation(
        first(terms), context, T
    )::ProposalEvaluation{T}
