    ...
    return map(descriptors) do descriptor
        ...
        if descriptor isa Union{
            ...
            ownership = LocalMath.Field(
                LocalMath.Space(_ContactSite, Tuple(source.domain.shape)), Int32)
            ...
            declaration = _spatial_relation_query_declaration(
                descriptor, source.domain_resources, Tuple(source.domain.shape),
                cartesian_periodic_axes(source.domain), length(workspace.staged_cell_kinds);
