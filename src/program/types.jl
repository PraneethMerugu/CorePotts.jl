...
function CompiledPottsProgram(
    ...
    attempts_per_site <= div(
        typemax(Int32), max(length(domain.mutable_sites), 1)
    ) || throw(ArgumentError(
        "mutable-site attempt count exceeds the positive Int32 identity domain"
    ))
    engine isa CheckerboardProgramEngine &&
        attempts_per_site > typemax(UInt8) && throw(ArgumentError(
            "checkerboard attempt rounds exceed the semantic RNG subround domain"
        ))
    _validate_contact_measure_aliases(
        descriptor_plan.domain_resources,
        domain.shape,
        cartesian_periodic_axes(domain),
    )
    ...
    _validate_stage_spatial_queries(
        ...
        descriptor_plan.domain_resources, domain)
